...
const IntroBasic = props => {
  ...
  const panelTimeline = (left, right) => {
    ...
    tl.from(left, 0.3, {
      ...
      .from(right, 0.3, { scaleX: 0, transformOrigin: '0% 50%' }, '-=0.3')
      .staggerFrom([homeTeamName, awayTeamName], 0.5, { autoAlpha: 0 });

    return tl;
  };

  const graphicsTimeLine = (homeLogo, versusBadge, awayLogo, penaltyCount) => {
    let tl = new TimelineMax();
    tl.from(homeLogo, 0.3, { autoAlpha: 0, transformOrigin: '50% 50%', scale: 0 })
      ...
      .from(penaltyCount, 0.3, { autoAlpha: 0, y: 100 })
  ...
  useEffect(() => {
    ...
    rootTimeline.add(graphicsTimeLine(htLogo, versusBadge, atLogo, penaltyCount));
  ...
  useEffect(() => {
    ...
    const defaultAnimation = animation.find(({ animation }) => animation === 'IntroV1');
    ...
    <div>
      <svg width="1000" height="500" viewBox="0 0 1000 500" version="1.1" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg id="venue_location" width="400px" height="60px" viewBox="0 0 400 60" x="300" y="0">
          <g ref={e => (penaltyCount = e)} className="location-name" strokeWidth="1" fillRule="evenodd">
